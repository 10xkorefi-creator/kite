  ...
  return (
    <section className="relative overflow-hidden py-12 lg:pb-32 bg-[#fafafb]/40">
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-4 pb-12 pt-4 sm:px-6 lg:grid-cols-12 lg:gap-12 lg:pb-20 lg:pt-8">
        ...
        {/* Right Column: Hero Image with Auto-playing Slider */}
        <motion.div
          initial={{
            opacity: 0,
            y: reduceMotion ? 0 : 30,
        ...
        >
          ...
          <div className="relative overflow-hidden rounded-2xl border border-ink/10 bg-white p-2 shadow-xl shadow-ink/5">
            <div className="relative aspect-[4/3] w-full overflow-hidden rounded-xl bg-slate-50">
              ...
              {/* Dots navigation overlay for slider */}
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-10 bg-slate-900/30 backdrop-blur-xs px-2.5 py-1.5 rounded-full">
                {slides.map((_, idx) => (
                  <button
                    key={idx}
                    onClick={() => setCurrentSlide(idx)}
                    className={`h-2 rounded-full transition-all duration-300 ${
                      currentSlide === idx ? "w-5 bg-white" : "w-2 bg-white/50 hover:bg-white/70"
                    }`}
